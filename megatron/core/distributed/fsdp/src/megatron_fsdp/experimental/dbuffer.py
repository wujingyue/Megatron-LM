...
def _split_range(
    ...
    """Per-rank (offset, numel) when one mesh axis of size ``dp_size`` shards [offset, offset + numel)."""
...
def _validate_placements(placements: Iterable[Placement]) -> None:
    ...
    for placement in placements:
        ...
        if isinstance(placement, TensorAtomic) and len(placements) > 1:
            raise NotImplementedError(
                f"Currently, DBuffer only supports only 1d device mesh, but got ndim={len(placements)}."
            )
