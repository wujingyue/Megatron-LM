...
class GlobalLayout:
    ...
    def build(
        ...
        if not isinstance(reference, (Flat, BlockAtomic, TensorAtomic)):
            raise ValueError(
                f"'reference' type should be chosen from (Flat, BlockAtomic, TensorAtomic), but got {type(reference)}."
            )
    ...
    def _build_for_tensor_atomic(
        ...
        if len(tensor_shapes) != len(tensor_to_owner_rank):
            raise ValueError(
                f"In planning a layout for TensorAtomic placement, the number of tensors must equal to the number of assignment entries, "
                f"but got {len(tensor_shapes)} and {len(tensor_to_owner_rank)}."
            )

        if set(tensor_to_owner_rank.keys()) != set(range(len(tensor_shapes))):
            raise ValueError(
                f"In planning a layout for TensorAtomic placement, the keys of 'tensor_to_owner_rank' must exactly match the indices of tensors of range ({len(tensor_shapes)}), "
        ...
        if min(candidate_rank_set) < 0 or max(candidate_rank_set) >= dp_size:
            raise ValueError(
                f"In planning a layout for TensorAtomic placement, the owner of each tensor must be within the range of data parallel size: [0, {dp_size}), "
                f"but got {min(candidate_rank_set)} for mininum, and {max(candidate_rank_set)} for maximum."
            )
    ...
    def __post_init__(self) -> None:
        ...
        if isinstance(self.reference, TensorAtomic):
            ...
            if sum(per_rank) != self.size:
                raise AssertionError(
                    f"For TensorAtomic reference placement, "
                    f"the sum of all ranks' local elements ({sum(per_rank)}) should equal to total size ({self.size})."
                )
            ...
            for tensor_id, (shape, start) in enumerate(
                ...
                if not (lo <= start and start + shape.numel() <= hi):
                    raise AssertionError(
                        f"Tensor {tensor_id} [{start}, {start + shape.numel()}) is out of owner rank {rank} segment [{lo}, {hi})."
                    )
